...
const Port = ({ q, r, type, direction }) => {
  ...
  return (
    <Hexagon q={q} r={r} s={-q-r} className={`port ${type} ${direction} port-hexagon`} stroke="none">
      <g className="port-content" transform={`rotate(${direction === 'W' ? 270 : direction === 'NW' ? 330 : direction === 'NE' ? 30 : direction === 'E' ? 90 : direction === 'SE' ? 150 : direction === 'S' ? 240 : direction === 'SW' ? 210 : 360})`}>
        {getPortIcon()}
      </g>
    </Hexagon>
  );
